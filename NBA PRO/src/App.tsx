  ...
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 space-y-3">
       <div className="flex justify-between items-center pb-2 border-b border-slate-200/50">
          ...
          <span className="text-[10px] font-black text-blue-600 tabular-nums bg-blue-50 px-2 py-0.5 rounded">{score} / {sub.maxMarks}</span>
       </div>

       <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {sub.subCriteria?.map(child => (
            <div key={child.id} className="bg-white p-2 rounded-md border border-slate-100 flex flex-col gap-2">
               <div className="flex justify-between items-center">
                  <span className="text-[9px] font-bold text-slate-400">{child.id}</span>
                  <span className="text-[9px] font-bold text-slate-400 italic">MAX {child.maxMarks}</span>
               </div>
               <div className="space-y-1.5">
                  {child.guidelines?.map(g => (
                    <GuidelineDensityInput 
                      key={g.id} 
                      guideline={g} 
                      value={scores[g.id] || 0} 
          ...
          {!sub.subCriteria && !sub.guidelines && (
             <GuidelineDensityInput 
               guideline={{ id: sub.id, description: "Direct Assessment Value", maxMarks: sub.maxMarks }} 
...
function GuidelineDensityInput({ guideline, value, onChange }: { guideline: any, value: number, onChange: (v: number) => void }) {
  return (
    <div className="flex justify-between items-center group">
      <label className="text-[9px] text-slate-500 uppercase font-black truncate w-40" title={guideline.description}>{guideline.description}</label>
      <div className="flex items-center gap-1">
         <button 
          onMouseDown={() => onChange(value - 1)}
          className="w-4 h-4 bg-slate-100 text-slate-400 rounded text-[10px] flex items-center justify-center hover:bg-slate-200 transition select-none"
         >
           -
         </button>
         <input 
          ...
          className="w-8 text-[10px] font-black border border-slate-200 rounded text-center bg-slate-50 focus:bg-white focus:outline-none focus:border-blue-300 py-0.5 tabular-nums" 
         />
         <button 
          onMouseDown={() => onChange(value + 1)}
          className="w-4 h-4 bg-slate-100 text-slate-400 rounded text-[10px] flex items-center justify-center hover:bg-blue-500 hover:text-white transition select-none"
         >
           +
         </button>
         <span className="text-[8px] text-slate-300 font-bold ml-1 w-4">/{guideline.maxMarks}</span>
      </div>
    </div>
  );
}
